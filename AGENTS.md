...
## Hard rules for Codex in this repo
...
- **Codex integration:** three workflows under `.github/workflows/` add bots — `Codex-pr-review.yml` does auto-review on PR open/sync (single edit-in-place summary tagged `[Codex-review]`), `Codex-implement-fixes.yml` handles `@Codex fix` and addresses feedback in one coalesced commit (replies tagged `[Codex-fix]`), and `extract-Codex-lessons.yml` does post-merge lessons extraction into `chore(Codex): learn from #N` PRs.
